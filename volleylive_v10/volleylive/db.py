...
CREATE TABLE IF NOT EXISTS lineups (id INTEGER PRIMARY KEY, match_id INTEGER NOT NULL, set_number INTEGER NOT NULL, team TEXT NOT NULL, players_json TEXT NOT NULL, libero_id INTEGER, UNIQUE(match_id,set_number,team));
CREATE TABLE IF NOT EXISTS subs_counter (id INTEGER PRIMARY KEY, match_id INTEGER NOT NULL, set_number INTEGER NOT NULL, team TEXT NOT NULL, used INTEGER NOT NULL DEFAULT 0, UNIQUE(match_id,set_number,team));
"""

def get_db():
    db_path = current_app.config.get("DB_PATH") or os.path.join(os.path.dirname(__file__), "matches.db")
...
def init_db(app):
    with app.app_context():
        ...
        if c.fetchone()[0] == 0:
            ...
            # equipos demo
            c.execute("INSERT INTO teams(name,short) VALUES (?,?)", ("Iberá V.C.","IBR")); home_id = c.lastrowid
            c.execute("INSERT INTO teams(name,short) VALUES (?,?)", ("Independiente","IND")); away_id = c.lastrowid
            # torneo + relación equipos
            ...
            tid = c.lastrowid
            c.execute("INSERT INTO tournament_teams(tournament_id,team_id) VALUES (?,?)",(tid,home_id))
            c.execute("INSERT INTO tournament_teams(tournament_id,team_id) VALUES (?,?)",(tid,away_id))
            # partido demo
            c.execute("""INSERT INTO matches(tournament_id,home_id,away_id,date,gym,status,rules_snapshot_json)
                         VALUES (?,?,?,?,?,?,?)""",
                      (tid,home_id,away_id,"2025-08-16T18:00:00","Polideportivo","scheduled",json.dumps(rules)))
            # planteles demo
            for rec in [(home_id,1,"Local 1","OH",0),(home_id,2,"Local 2","MB",0),(home_id,3,"Local 3","S",0),
                        (home_id,4,"Local 4","OP",0),(home_id,5,"Local 5","OH",0),(home_id,6,"Local 6","MB",0),
                        (home_id,12,"Líbero Local","L",1),(home_id,7,"Local 7","OH",0),(home_id,8,"Local 8","MB",0)]:
                c.execute("INSERT INTO players(team_id,number,name,role,libero) VALUES (?,?,?,?,?)", rec)
            for rec in [(away_id,1,"Visita 1","OH",0),(away_id,2,"Visita 2","MB",0),(away_id,3,"Visita 3","S",0),
                        (away_id,4,"Visita 4","OP",0),(away_id,5,"Visita 5","OH",0),(away_id,6,"Visita 6","MB",0),
                        (away_id,10,"Líbero Visita","L",1),(away_id,7,"Visita 7","OH",0),(away_id,8,"Visita 8","MB",0)]:
                c.execute("INSERT INTO players(team_id,number,name,role,libero) VALUES (?,?,?,?,?)", rec)
