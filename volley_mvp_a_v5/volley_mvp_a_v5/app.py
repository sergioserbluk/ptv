...
@app.route("/")
def index(): return "OK · Control: /control · Display: /display"
...
def api_tournaments():
    conn = get_db(); c = conn.cursor()
    c.execute("""SELECT t.id, t.name, t.season, r.name
                 FROM tournaments t JOIN rulesets r ON r.id=t.ruleset_id""")
    rows = [{"id": r[0], "name": r[1], "season": r[2], "ruleset": r[3]} for r in c.fetchall()]
...
def api_matches():
    ...
    conn = get_db(); c = conn.cursor()
    if tid:
        c.execute("""SELECT m.id, th.name, ta.name, m.status FROM matches m
                     JOIN teams th ON th.id=m.home_id
                     JOIN teams ta ON ta.id=m.away_id
        ...
        c.execute("""SELECT m.id, th.name, ta.name, m.status FROM matches m
                     ...
                     JOIN teams ta ON ta.id=m.away_id
                     ORDER BY m.id DESC""")
    rows = [{"id": r[0], "label": f"{r[1]} vs {r[2]}", "status": r[3]} for r in c.fetchall()]
    conn.close(); return jsonify(rows)
