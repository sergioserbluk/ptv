...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        # Seed players
        c.execute("INSERT INTO players(team_id, number, name, role, libero) VALUES (?,?,?,?,?)", (home_id, 1, "Jugador Local 1", "OH", 0))
        c.execute("INSERT INTO players(team_id, number, name, role, libero) VALUES (?,?,?,?,?)", (home_id, 2, "Jugador Local 2", "MB", 0))
        c.execute("INSERT INTO players(team_id, number, name, role, libero) VALUES (?,?,?,?,?)", (home_id, 12, "Líbero Local", "L", 1))
        c.execute("INSERT INTO players(team_id, number, name, role, libero) VALUES (?,?,?,?,?)", (away_id, 5, "Jugador Visita 1", "S", 0))
...
def api_players():
    ...
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT id, number, name, role, libero FROM players WHERE team_id=? ORDER BY number", (team_id,))
...
# ---- APIs creación/edición/borrado (admin) ----
...
@app.route("/api/admin/team/<int:team_id>", methods=["PUT","DELETE"])
def api_admin_team_ud(team_id):
    conn = get_db(); c = conn.cursor()
    if request.method == "PUT":
        data = request.get_json(force=True)
        name = (data.get("name") or "").strip()
        short = (data.get("short") or "").strip() or None
        if not name: conn.close(); return jsonify({"error":"name requerido"}), 400
        c.execute("UPDATE teams SET name=?, short=? WHERE id=?", (name, short, team_id))
        conn.commit(); conn.close(); return jsonify({"ok": True})
    # DELETE: bloquear si hay referencias
    c.execute("SELECT 1 FROM matches WHERE home_id=? OR away_id=?", (team_id, team_id))
    if c.fetchone(): conn.close(); return jsonify({"error":"No se puede borrar: el equipo está en partidos"}), 400
    c.execute("SELECT 1 FROM players WHERE team_id=?", (team_id,))
    if c.fetchone(): conn.close(); return jsonify({"error":"No se puede borrar: el equipo tiene jugadores"}), 400
...
@app.route("/api/admin/player/<int:player_id>", methods=["PUT","DELETE"])
def api_admin_player_ud(player_id):
    conn = get_db(); c = conn.cursor()
    if request.method == "PUT":
        d = request.get_json(force=True)
        number = d.get("number")
        ...
        libero = 1 if d.get("libero") else 0
        if not name: conn.close(); return jsonify({"error":"name requerido"}), 400
        c.execute("UPDATE players SET number=?, name=?, role=?, libero=? WHERE id=?", (number, name, role, libero, player_id))
        conn.commit(); conn.close(); return jsonify({"ok": True})
...
@app.route("/api/admin/tournament/<int:tournament_id>", methods=["PUT","DELETE"])
def api_admin_tournament_ud(tournament_id):
    conn = get_db(); c = conn.cursor()
    if request.method == "PUT":
        d = request.get_json(force=True)
        name = (d.get("name") or "").strip()
        season = (d.get("season") or "").strip()
        ttype = (d.get("type") or "round_robin").strip()
        if not name: conn.close(); return jsonify({"error":"name requerido"}), 400
        c.execute("UPDATE tournaments SET name=?, season=?, type=? WHERE id=?", (name, season, ttype, tournament_id))
        conn.commit(); conn.close(); return jsonify({"ok": True})
    # DELETE: bloquear si tiene partidos
    c.execute("SELECT 1 FROM matches WHERE tournament_id=?", (tournament_id,))
    if c.fetchone(): conn.close(); return jsonify({"error":"No se puede borrar: el torneo tiene partidos"}), 400
...
@app.route("/api/admin/match/<int:match_id>", methods=["PUT","DELETE"])
def api_admin_match_ud(match_id):
    conn = get_db(); c = conn.cursor()
    if request.method == "PUT":
        d = request.get_json(force=True)
        date = d.get("date"); gym = d.get("gym"); status = d.get("status")
        c.execute("UPDATE matches SET date=?, gym=?, status=? WHERE id=?", (date, gym, status, match_id))
        conn.commit(); conn.close(); return jsonify({"ok": True})
    # DELETE: bloquear si tiene eventos
    c.execute("SELECT 1 FROM events WHERE match_id=?", (match_id,))
    if c.fetchone(): conn.close(); return jsonify({"error":"No se puede borrar: el partido tiene eventos registrados"}), 400
